...
//xiaoya
static DWORD register_desktopInfo_entry(const char *desktop_name, const char *desktop_info)
{
    WCHAR *desktop_nameW;
    WCHAR *desktop_infoW;
    INT size;
    DWORD ret;
    size = MultiByteToWideChar(CP_UNIXCP, 0, desktop_name, -1, NULL, 0);
    desktop_nameW = HeapAlloc(GetProcessHeap(), 0, size * sizeof(WCHAR));
    if (desktop_nameW)
    {
        MultiByteToWideChar(CP_UNIXCP, 0, desktop_name, -1, desktop_nameW, size);
        size = MultiByteToWideChar(CP_UNIXCP, 0, desktop_info, -1, NULL, 0);
        desktop_infoW = HeapAlloc(GetProcessHeap(), 0, size * sizeof(WCHAR));
        if (desktop_infoW)
        {
...
static BOOL WaitForParentProcess( int pid )
{
    ...
    fprintf(stderr,"WaitForMultipleObject\n");
...
static BOOL Process_Link( LPCWSTR linkname, BOOL bWait, int pid )
{
    IShellLinkW *sl;
    IPersistFile *pf;
    HRESULT r;
...
static BOOL Process_URL( LPCWSTR urlname, BOOL bWait, int pid )
{
...
static const WCHAR winemenubuilderServiceW[] = {'w','i','n','e','m','e','n','u','b','u','i','l','d','e','r','S','e','r','v','i','c','e',0};
