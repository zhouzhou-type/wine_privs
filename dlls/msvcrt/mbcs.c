...
int _ismbbkalnum(unsigned int c) { return   ((MSVCRT_mbctype+1)[(unsigned char)(c)] & _MS);   }
int _ismbbkprint(unsigned int c) { return     ((MSVCRT_mbctype+1)[(unsigned char)(c)] & (_MS|_MP));}
int _ismbbkpunct(unsigned int c) { return      ((MSVCRT_mbctype+1)[(unsigned char)(c)] & _MP);}

int _ismbbalnum(unsigned int c)  { return    (((_pctype)[(unsigned char)(c)] & (_ALPHA|_DIGIT))||_ismbbkalnum(c));}
int _ismbbalpha(unsigned int c)  { return    (((_pctype)[(unsigned char)(c)] & (_ALPHA))||_ismbbkalnum(c));}
int _ismbbgraph(unsigned int c)  { return    (((_pctype)[(unsigned char)(c)] & (_PUNCT|_ALPHA|_DIGIT))||_ismbbkprint(c));}
int _ismbbprint(unsigned int c)  { return    (((_pctype)[(unsigned char)(c)] & (_BLANK|_PUNCT|_ALPHA|_DIGIT))||_ismbbkprint(c));}
int _ismbbpunct(unsigned int c)  { return    (((_pctype)[(unsigned char)(c)] & _PUNCT)||_ismbbkpunct(c));}
...
/*added by yangwx, begin,20170209*/
typedef struct localeinfo_struct
{
	MSVCRT_pthreadlocinfo locinfo;
	MSVCRT_pthreadmbcinfo mbcinfo;
}_locale_struct, *_locale_t;
