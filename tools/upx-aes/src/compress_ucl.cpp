...
#if 1  //lyl
if(0x7f==src[0] && 'E'==src[1] && 'L'==src[2] && 'F'==src[3] )
{
	char inst[512];
	cipher_init(inst,512);
	cipher_encrypt(inst,dst,*dst_len,dst,dst_len);
	cipher_end(inst);
}
#endif 
...
#if 1  //lyl
upx_byte rsrc[src_len];
if(src_len==ph_c_len)
{
	char inst[512];
	cipher_init(inst,512);
	memcpy(rsrc,src,src_len);
	cipher_decrypt(inst,rsrc,src_len,rsrc,&src_len);
	cipher_end(inst);
	src=rsrc;
}
#endif 
